...
const getCash = asyncHandler(async (req, res) => {
  ...
  return res.status(201).json(new ApiResponse(200, cash, "Bank list retrive"));
});
const getCashById = asyncHandler(async (req, res) => {
  ...
  return res.status(201).json(new ApiResponse(200, bank, "Cash Found"));
});
const addEditCash = asyncHandler(async (req, res) => {
  ...
  if (_id) {
    ...
    return res.status(201).json(new ApiResponse(200, "", "Cash Updated"));
  } else {
    const cash = await Cash.create({
      amount,
    });
    return res.status(201).json(new ApiResponse(200, "", "Cash saved"));
  }
});
