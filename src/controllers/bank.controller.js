...
const getBanks = asyncHandler(async (req, res) => {
  ...
  return res
    .status(201)
    .json(new ApiResponse(200, bank, "Bank list retrive"));
});
const getBankById = asyncHandler(async (req, res) => {
  ...
  return res
    .status(201)
    .json(new ApiResponse(200, bank, "Bank Found"));
});
const totalBankBalace = asyncHandler(async (req, res) => {
  const banks = await Bank.find();
  const totalBalance = banks.reduce((total, bank) => total + parseFloat(bank.balance), 0);  return res
    .status(201)
...
const addEditBanks = asyncHandler(async (req, res) => {
  ...
  if (_id) {
    ...
    return res
    .status(201)
    .json(new ApiResponse(200, "", "Bank Updated"));
    ...
    return res
    .status(201)
    .json(new ApiResponse(200, "", "Bank Saved"));
  }
});
